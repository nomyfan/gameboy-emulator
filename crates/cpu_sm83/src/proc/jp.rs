...
mod tests {
    ...
    fn jp_condition_success() {
        let cases = [
            ...
            (0xD2, Am::Sixteen, Some(Condition::NC), (false, true, true, false)),
        ];
    ...
    fn jp_condition_fail() {
        let mut mock = MockCpu16::new();
        mock.expect_fetch_data().with(eq(Am::Sixteen)).once().return_const(1u16);
        mock.expect_flags().once().returning(move || (false, true, true, true));

        assert_eq!(proc_jp(&mut mock, 0xCA, &Some(Condition::Z), &Am::Sixteen), get_cycles(0xCA).1)
    }
}
